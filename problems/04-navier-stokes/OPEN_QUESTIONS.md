...
| 2 | quelle équicontinuité temporelle critique garantit une trace fortement compacte dans une extraction de premier blow-up, sans supposer la borne globale `L³` qui ferme déjà ESS/GKP ? | le cycle 0004 fournit une contre-suite de solutions NS exactes aux temps `t_N~N^-2`; le défaut disparaît dans l'intérieur espace-temps | lemme de trace uniforme compatible avec les seules bornes effectivement héritées, ou contre-suite à cette hypothèse |
