...
|---:|---|---|---|
| 1 | un blob critique compact peut-il rendre sa direction interne asymptotiquement constante tout en compensant `integral curl U=0` sur une région opposée de volume décroissant ? | le cycle 0025 réalise divergence, énergie, faible-`L^(3/2)` et masse par bloc, mais toute forme directionnelle fixe échoue au log-BMO global; seule une intermittence imbriquée peut encore aplatir le motif | construction div–curl à deux amplitudes avec oscillation `O(1/n)` et résidu suivi, ou minoration coercive uniforme depuis l'annulation vectorielle et un contrôle critique d'amplitude |
